...
Citizen.CreateThread(function()
    while true do
		...
		if betweencoords < 2.0 and speaked == false then
			DrawTxt(" PRESS [~e~ENTER~q~] to speak to Cliff's Son Roger ", 0.50, 0.90, 0.7, 0.7, true, 255, 255, 255, 255, true)
...
Citizen.CreateThread(function()
    while true do
		...
		if betweencoords < 2.0 and placedbait == true then
			DrawTxt(" PRESS [~e~ENTER~q~] to speak to Roger again ", 0.50, 0.90, 0.7, 0.7, true, 255, 255, 255, 255, true)
			if IsControlJustReleased(0, 0xC7B5340A) then
                placedbait = false
				isfinished = true
...
AddEventHandler('talon_robbery:go3', function()	
	...
	if started == true then
		TriggerEvent("vorp:TipBottom", 'Go Search for Cliff ', 5000)
        Citizen.Wait(2000)
...
AddEventHandler("talon:ToggleNotification3", function()
    TriggerEvent("vorp:TipBottom", 'Search the Area for the Bear ', 15000)
